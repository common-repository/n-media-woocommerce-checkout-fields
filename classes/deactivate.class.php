...
class CFOM_Deactivate {
    
    
    public function __construct() {
        
        $cfom_dir_name  = basename(dirname(__DIR__));
        $cfom_basename  = "{$cfom_dir_name}/wc-checkout-fields.php";
	    add_filter( "plugin_action_links_{$cfom_basename}", array($this, 'cfom_deactivate_link'), 99);
        
        add_action('admin_enqueue_scripts', array($this, 'load_script'));
        
        add_action('admin_footer', array($this, 'deactivate_scripts'));
        add_action('wp_ajax_cfom_submit_uninstall_reason', array($this, "send_uninstall_reason"));
    }
    
    function cfom_deactivate_link($links) {
        
        
        if (array_key_exists('deactivate', $links)) {
            $links['deactivate'] = str_replace('<a', '<a class="cfom-deactivate-link"', $links['deactivate']);
        }
    ...
    public function deactivate_scripts() {
        ...
        <div class="cfom-deactivate-modal" id="cfom-deactivate-modal">
            <div class="cfom-deactivate-modal-wrap">
                <div class="cfom-deactivate-modal-header">
                    <h3><?php _e('Please Help Us to Make it Better for You.', 'product-import-export-for-woo'); ?></h3>
                </div>
                <div class="cfom-deactivate-modal-body">
                    <ul class="reasons">
                ...
                <div class="cfom-deactivate-modal-footer">
        ...
        <style type="text/css">
            ...
            .cfom-deactivate-modal.modal-active {display: block;}
            .cfom-deactivate-modal-wrap {
                width: 50%;
                position: relative;
                margin: 5% auto;
                background: #fff;
            }
            .cfom-deactivate-modal-header {
                border-bottom: 1px solid #eee;
                padding: 8px 20px;
            }
            .cfom-deactivate-modal-header h3 {
                line-height: 150%;
                margin: 0;
            }
            .cfom-deactivate-modal-body {padding: 5px 20px 20px 20px;}
            .cfom-deactivate-modal-body .input-text,.cfom-deactivate-modal-body textarea {width:75%;}
            .cfom-deactivate-modal-body .reason-input {
                margin-top: 5px;
                margin-left: 20px;
            }
            .cfom-deactivate-modal-footer {
    ...
    function load_script($hook) {
		...
		// Preloader script
        wp_enqueue_script('cfom-deactivate', CFOM_URL."/js/cfom-deactivate.js", array('jquery'), CFOM_VERSION, true);
	}
