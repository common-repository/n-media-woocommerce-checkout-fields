...
function get_option(key) {
	...
	jQuery.each(cfom_vars.settings, function(k, option) {
...
function cfom_addThousandSeperator(n){

    var rx=  /(\d+)(\d{3})/;
    return String(n).replace(/^\d+/, function(w){
        while(rx.test(w)){
            w= w.replace(rx, '$1'+cfom_vars.wc_thousand_sep+'$2');
        }
        return w;
    });
...
function get_woocommerce_price_format(p) {
  var currency_pos = cfom_vars.wc_currency_pos;
  var format = '';
  var sym = cfom_vars.woo_currency;
...
function cfom_update_quantity() {
	...
	var decimalSeparator = cfom_vars.wc_decimal_sep;
	var noOfDecimal = cfom_vars.wc_no_decimal;
	

	//resetting
	jQuery('#display-total-price > span.cfom-total-option-price').hide();
	jQuery('#display-total-price > span.cfom-grand-total-price').hide();
	
	jQuery('#input-quantities').find('.cfom-quantity').each(function(){
		...
		html_total_price = cfom_addThousandSeperator(total_price.toFixed(noOfDecimal));
		html_total_price = html_total_price.toString().replace('.', decimalSeparator);
	});
	
	// Product Total
	jQuery('#display-total-price > span.cfom-total-price').show().find('.cfom-price').html( html_total_price );
	
	var grand_total = parseFloat(total_option_price) + parseFloat(total_price);
	// Option Total
	if( total_option_price > 0 ){
		
		var html_total_option_price = cfom_addThousandSeperator(total_option_price.toFixed(noOfDecimal));
		html_total_option_price = html_total_option_price.toString().replace('.', decimalSeparator);
		jQuery('#display-total-price > span.cfom-total-option-price').show().find('.cfom-price').html( html_total_option_price );
		
		var html_grand_total = cfom_addThousandSeperator(grand_total.toFixed(noOfDecimal));
		html_grand_total = html_grand_total.toString().replace('.', decimalSeparator);
		jQuery('#display-total-price > span.cfom-grand-total-price').show().find('.cfom-price').html( html_grand_total );
	}
